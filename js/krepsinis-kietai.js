...
const scores = new Array(resultsDOM.length).fill(0);
...
for (let i = 0; i < buttonsDOM.length; i++) {
    buttonsDOM[i].addEventListener('click', () => {
        ...
        historyDOM.insertAdjacentHTML('afterbegin', `<div class="entry ${team ? 'away' : 'home'}">${team ? 'Svečių' : 'Namų'} komanda pelnė ${points} tašk${points -1 ? 'us' : 'ą'}.</div>`);
    })
}